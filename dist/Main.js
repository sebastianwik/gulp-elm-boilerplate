...
Elm.Counter.make = function (_elm) {
   ...
   var Context = F2(function (a,b) {
      return {actions: a,remove: b};
   });
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "Increment") {
            return _U.cmp(model.count,10) < 0 ? _U.update(model,
            {count: model.count + 1}) : model;
         } else {
            return _U.cmp(model.count,0) > 0 ? _U.update(model,
            {count: model.count - 1}) : model;
         }
   });
   ...
   var viewWithRemoveButton = F2(function (context,model) {
      ...
      _U.list([]),
      _U.list([A2($Html.button,
              _U.list([A2($Html$Events.onClick,context.actions,Decrement)]),
              _U.list([$Html.text("-")]))
              ,A2($Html.div,
              _U.list([countStyle]),
              _U.list([$Html.text($Basics.toString(model.count))]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,context.actions,Increment)]),
              _U.list([$Html.text("+")]))
              ,A2($Html.button,
   ...
   var init = function (n) {    return {count: n};};
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: $CounterList.init
